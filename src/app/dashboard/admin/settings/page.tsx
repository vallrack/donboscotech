...
export default function SettingsPage() {
  ...
  const handleDelete = async (col: string, id: string) => {
    ...
    try {
      ...
      toast({ title: "Eliminado", description: "El elemento ha sido removido." });
    } catch (e) {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div>
        ...
        <p className="text-muted-foreground">Gestiona sedes, programas académicos y jornadas laborales.</p>
      </div>
      ...
      <Tabs defaultValue="sedes" className="w-full">
        ...
        <TabsContent value="sedes" className="mt-6">
          <Card className="border-none shadow-xl rounded-3xl">
            <CardHeader>
              <CardTitle>Sedes de Ciudad Don Bosco</CardTitle>
              <CardDescription>Agregar o eliminar sedes institucionales.</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="flex gap-4 mb-6">
                <Input id="new-campus" placeholder="Nombre de la sede (Ej: Sede Norte)" className="h-12 rounded-xl" />
                <Button onClick={() => {
              ...
              <div className="grid gap-3">
                {campuses?.map(c => (
                  <div key={c.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl border">
                    <span className="font-bold">{c.name}</span>
                    <Button variant="ghost" size="icon" className="text-destructive" onClick={() => handleDelete('campuses', c.id)}><Trash2 className="w-4 h-4" /></Button>
                  </div>
                ))}
              </div>
            </CardContent>
        ...
        <TabsContent value="programas" className="mt-6">
          <Card className="border-none shadow-xl rounded-3xl">
            <CardHeader>
              <CardTitle>Programas y Carreras</CardTitle>
              <CardDescription>Especialidades impartidas por los docentes.</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="flex gap-4 mb-6">
                <Input id="new-program" placeholder="Nombre del programa (Ej: Mecánica)" className="h-12 rounded-xl" />
                <Button onClick={() => {
              ...
              <div className="grid gap-3">
                {programs?.map(p => (
                  <div key={p.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl border">
                    ...
                    <Button variant="ghost" size="icon" className="text-destructive" onClick={() => handleDelete('programs', p.id)}><Trash2 className="w-4 h-4" /></Button>
                  </div>
                ))}
              </div>
            </CardContent>
        ...
        <TabsContent value="jornadas" className="mt-6">
          <Card className="border-none shadow-xl rounded-3xl">
            <CardHeader>
              ...
              <CardDescription>Configuración de horarios de entrada y salida.</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8 p-6 bg-primary/5 rounded-3xl">
                <Input id="shift-name" placeholder="Nombre (Ej: Mañana)" className="h-12 rounded-xl" />
                <Input id="shift-start" type="time" className="h-12 rounded-xl" />
                <Input id="shift-end" type="time" className="h-12 rounded-xl" />
                <Button className="md:col-span-3 h-12 rounded-xl font-bold" onClick={() => {
                  ...
                  if (name && start && end) handleAddItem('shifts', { name, startTime: start, endTime: end, days: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri'] });
                }} disabled={loading}>
                  Configurar Nueva Jornada
                </Button>
              </div>
              <div className="grid gap-4">
                {shifts?.map(s => (
                  <div key={s.id} className="p-5 bg-white rounded-2xl border shadow-sm flex items-center justify-between">
                    <div>
                      <p className="font-black text-lg text-primary">{s.name}</p>
                      <p className="text-sm font-medium text-muted-foreground flex items-center gap-2">
                        <Clock className="w-3.5 h-3.5" /> {s.startTime} - {s.endTime}
                      </p>
                    </div>
                    <Button variant="ghost" size="icon" className="text-destructive" onClick={() => handleDelete('shifts', s.id)}><Trash2 className="w-4 h-4" /></Button>
                  </div>
                ))}
              </div>
            </CardContent>
