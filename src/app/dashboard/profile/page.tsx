        ...
        <div className="lg:col-span-4 space-y-8">
          <Card className="border-none shadow-2xl rounded-[3rem] bg-white p-10 text-center flex flex-col items-center">
             <div className="relative group cursor-pointer" onClick={() => !saving && fileInputRef.current?.click()}>
                <div className="w-40 h-40 rounded-[3rem] bg-gray-50 flex items-center justify-center overflow-hidden border-4 border-white shadow-xl relative">{formData.avatarUrl ? (<Image src={formData.avatarUrl} alt="Avatar" width={160} height={160} className="object-cover w-full h-full" unoptimized />) : (<UserIcon className="w-16 h-16 text-gray-200" />)}<div className="absolute inset-0 bg-primary/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center text-white gap-2"><Camera className="w-8 h-8" /><span className="text-[8px] font-black uppercase">Foto</span></div></div>
          ...
          <Card className="border-none shadow-2xl rounded-[3rem] bg-white p-10 flex flex-col items-center gap-4">
             ...
             <div className="w-full aspect-video bg-gray-50 rounded-2xl border-2 border-dashed border-gray-200 flex items-center justify-center overflow-hidden relative group cursor-pointer" onClick={() => !saving && sigInputRef.current?.click()}>{formData.signatureUrl ? (<img src={formData.signatureUrl} alt="Firma" className="max-w-full max-h-full object-contain p-4" />) : (<div className="flex flex-col items-center text-muted-foreground/40 gap-2"><PenTool className="w-8 h-8" /><span className="text-[10px] font-bold">Subir firma</span></div>)}</div>
          ...
          <Card className="border-none shadow-2xl rounded-[3rem] bg-white overflow-hidden">
             <form onSubmit={handleSave}>
                ...
                <CardContent className="p-10 space-y-8">
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-8"><div className="space-y-3"><Label className="text-[10px] font-black uppercase tracking-[0.2em]">Nombre Completo</Label><Input value={formData.name} onChange={(e) => updateField('name', e.target.value)} className="h-14 rounded-2xl bg-gray-50/50 font-bold" required /></div><div className="space-y-3"><Label className="text-[10px] font-black uppercase tracking-[0.2em]">Cédula</Label><Input value={formData.documentId} onChange={(e) => updateField('documentId', e.target.value)} className="h-14 rounded-2xl bg-gray-50/50 font-bold" required /></div><div className="space-y-3"><Label className="text-[10px] font-black uppercase tracking-[0.2em]">Sede</Label><Select value={formData.campus || undefined} onValueChange={(v) => updateField('campus', v)}><SelectTrigger className="h-14 rounded-2xl bg-gray-50/50 font-bold"><SelectValue placeholder="Sede" /></SelectTrigger><SelectContent>{campuses.map(c => <SelectItem key={c.id} value={c.name}>{c.name}</SelectItem>)}</SelectContent></Select></div><div className="space-y-3"><Label className="text-[10px] font-black uppercase tracking-[0.2em]">Programa</Label><Select value={formData.program || undefined} onValueChange={(v) => updateField('program', v)}><SelectTrigger className="h-14 rounded-2xl bg-gray-50/50 font-bold"><SelectValue placeholder="Programa" /></SelectTrigger><SelectContent>{programs.map(p => <SelectItem key={p.id} value={p.name}>{p.name}</SelectItem>)}</SelectContent></Select></div></div>
